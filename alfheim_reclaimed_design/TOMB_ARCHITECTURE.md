...
**Status:** `residue budget corrected 2026-09-12; the composition rebuild is unstarted`.
**Authority:** subordinate to `INSTRUCTIONS.md` and `DEEPWORKS.md`. Governs
`tomb_centre`/`tomb_approach`/`tomb_wing` in `tools/gen_deep_archaeology.py` and the seventeen
...
Three templates — centre 47x22x47, approach 17x18x33, wing 47x22x47 — assembled by jigsaw into a
nine-piece complex roughly 200 blocks across, 312,025 source template blocks across the three
families. The vocabulary is there too, and it was expensive: pointed arches on both axes, alcoves,
...
Seven roles, each with its own builder and its own furniture rule, replacing the one loop:
...
| **2** | the room-role registry and relative furniture placement, wing first | a burial room's sarcophagus, statues, tapestry and carving band all derived from the room extent; guard 2 asserts no absolute placements remain |
| **3** | the single axis: `descent`/`seal` replace `four_doors` and the corridor cross | guard 3; the approach ends at a sealed door |
| **4** | register bands; the `%9`/`%11` lattice removed | guard 4, per burial room |
| **5** | `treasury` and `serdab`; debris re-placed as breach evidence | a runtime walk in a generated world |
...
2. **Should the burial chamber be corbelled?** A beehive vault is the single most recognizable
   Celtic move, and the pack has no corbelled dome helper yet.
...
5. **How much larger?** "Massive scale" is already ~200 blocks across. If the intent is larger
   still, that is a jigsaw `size` and piece-count change, and it should be settled before stage 2
   rather than after.
