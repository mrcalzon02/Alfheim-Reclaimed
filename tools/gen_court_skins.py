...
The former implementation reserved two global slots and policed every natural wood-elf spawn
with `16_wood_elf_skins.js`. The September field run showed that handler faulting in KubeJS's
shared scope, and the user explicitly rejected ongoing runtime scripts for cosmetic selection.
The legacy slot textures remain until the queued asset pass replaces them with a dedicated
court-NPC model/texture route; this generator no longer emits a runtime handler.
"""
import argparse
import glob
import os
...
def main():
    ...
    if not a.dry_run:
        if os.path.exists(sp):
            os.remove(sp)
    print(f'  no runtime reservation handler -> {sp}')
    print('\n  2 legacy slot textures retained pending the dedicated court-NPC asset pass')
    return 0
